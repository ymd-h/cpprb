...
By expanding Bellman equation, a target of Q function becomes $\sum
_{k=0}^{N-1} \gamma ^k r_{t+k} + \gamma ^N \max _{a} Q(s_{t+N},a)$.
...
then ~sample~ method returns (i+N)-th value instead of (i+1)-th one.


~sample~ also adds ~"discount"~ (\( \gamma ^{N-1} \)) into returned ~dict~.
