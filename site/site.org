...
| ~ctx~            | ~backend~      | internal data | synchronization objects |
|------------------+----------------+---------------+-------------------------|
| ~fork~ / ~spawn~ | ~sharedctypes~ | shared memory | OS semaphore            |
| ~fork~ / ~spawn~ | ~SharedMemory~ | shared memory | OS semaphore            |
| ~SyncManager~    | ~shreadctypes~ | proxy based   | proxy based             |
| ~SyncManager~    | ~ShreadMemory~ | shared memory | proxy based             |
