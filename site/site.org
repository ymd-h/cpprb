...
the current policy and those in replay buffer (taken from many
behavior policies).
...
The current distribution \( d^{\pi} \) is approximated by that in the
secondary small (fast) replay buffer \( d^{\mathcal{D}_f} \).

The estimation of the probability ratio is based on the following
Lemma;

\[ D(P\|Q) \geq \mathbb{E}_P[f^{\prime}(w(x))] - \mathbb{E}_Q[f^{\ast}(f^{\prime}(w(x)))] \]

, where \( f \) is any convex lower-semicontinuous function,
\(f^{\prime}\) is the first order derivative, and \(f^{\ast}\) is the
convex conjugate. The equality in the Lemma is achieved when
\(w=dP/dQ\).

By minimizing
\(L_{w}(\psi)=\mathbb{E}_{\mathcal{D}}[f^{\ast}(f^{\prime}(w_{\psi}(s,a)))]-\mathbb{E}_{\mathcal{D}_f}[f^{\prime}(w_{\psi}(s,a))]\),
\(w_{\psi}(s,a)\) approches to \(d^{\mathcal{D}_f}/d^{\mathcal{D}}\).
...
** With cpprb
You can implement LFIW with cpprb. You can use 2 replay buffers.
