...
** Overview
[[https://arxiv.org/abs/1906.08387][Experience Replay Optimization (ERO)]] masks stored transitions in order
...
In order to train replay policy, binary masks (\( \mathbf{I} \)) produced
by Bernoulli distribution are considered as action. The replay-reward
as defined as the difference between culmutive reward of the previous
policy (\( \pi ^{\prime} \)) and that of agent policy (\( \pi \)).
...
** References
- [[https://arxiv.org/abs/1906.08387][D. Zha et. al., "Experience Replay Optimization", (2019) arXiv cs.LG 1906.08387]]
