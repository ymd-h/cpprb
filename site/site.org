...
CLOSED: [2019-03-02 Sat 12:16]
:PROPERTIES:
:EXPORT_FILE_NAME: multiprocessing
:END:

#+begin_example
WARNING: Multi-Processing is beta feature. This might be buggy, and its API can be change without notice.
#+end_example

*** Overview
To speed up your explorarion task, you might want to run multiple
...
each process, so that we recreate replay buffer (interface of shared
memories) in ~init_worker~.
