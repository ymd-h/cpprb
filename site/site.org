...
~load_transitions~ does not overwrite existent transitions but adds,
...
(shared) memory acquired at Python side. For such memory management,
we use array specialized =std::shared_ptr=. [[http://clang.llvm.org/][clang]] does not support
=reset= method for specialized version. This is the reason we cannot
distribute binary wheel for Apple macOS. ([[https://gitlab.com/ymd_h/cpprb/-/issues/71][#71]])
...
too. The multi-processing version updates its tree lazily.
...
environment. =.coveragerc= defines coverage configuration.
...
=benchmark= and =benchmark2= directory contain benchmark script and
Dockerfile using at [[https://ymd_h.gitlab.io/cpprb/comparison/benchmark/][Benchmark page]]. These benchmarks might be outdated
...
=CITATION.cff= is used for citation management at GitHub.


=setup.py= used for package build. =MANIFEST.in= specifies package
additional files.
...
=crean_reinstall.sh= is old script. Recently, we don't use it.
...
In order to realize one command installation, NumPy and Cython are
lazily imported after =setup_requires= become available.

Exported markdown of README is re-used as =long_description= at PyPI
package page.
...
*** Documentation
Project site are build by [[https://gohugo.io/][Hugo]] and hosted at [[https://docs.gitlab.com/ee/user/project/pages/][GitLab Pages]]. The
...
[[https://ymd_h.gitlab.io/cpprb/][landing page]].

To show star and fork counts on the side bar, we use custom JavaScript
defined at =site/static/js/custom.js=.
...
codes. The docstring adopts [[https://numpydoc.readthedocs.io/en/latest/format.html][NumPy doc style]], so that [[https://sphinxcontrib-napoleon.readthedocs.io/en/latest/][Napoleon
extension]] is enabled. By using custom CSS
(aka. =sphinx/static/custom.css=), we try to match the look and feel
with that of the Hugo part. Additionally, [[https://sphinx-automodapi.readthedocs.io/en/latest/][sphinx-automodapi]] to build
the module structure. Unfortunately, there is an unfixed bug [[https://gitlab.com/ymd_h/cpprb/-/issues/118][#118]], all
methods are described twice (Why?).
...
*** CI Configuration
We mainly utilize [[https://docs.gitlab.com/ee/ci/][GitLab CI/CD]]. We execute build binary, unit test,
test coverage aggregation, upload to PyPI, project site build, and
project site deploy. The configuration is defined at =.gitlab-ci.yml=.

Additionally, [[https://github.co.jp/features/actions][GitHub Actions]] are used for specific purpose. We can use
Windows and macOS for free. We execute test script on them and upload
Windows binary wheel package to PyPI. When we fix the macOS build
problem, we will upload the binary wheel for macOS, too.
...
larger buffer maked learning slower. Combined Experience Replay (CER)
...
normalized over the main (slow) replay buffer with temerature
hyperparameter \(T\);
...
:PROPERTIES:
:EXPORT_HUGO_SECTION*: understanding
:EXPORT_FILE_NAME: _index
...
where the oldest transition is overwritten by a newly comming
...
target. If the objective loss is intermidiate of MSE and L1
