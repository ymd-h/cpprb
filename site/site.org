...
transition. The next sample containes one step older transions.
...
|-----------+------------------+---------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ~ctx~     | ~None~           | ~Optional[Union[ForkContext,SpawnContext,SyncManager]]~ | Context to be used for ~Event~, ~Lock~ etc. If ~None~ (default), default context is used. The context passed must match with the context of ~Process~. When ~SyncManager~ is passed, ~Event~ and ~Lock~ etc are accessed through proxy. |
| ~backend~ | ~"sharedctypes"~ | ~"sharedctypes"~ or ~"SharedMemory"~ (only for Python 3.8+) | Backend for shared memory. If ~"sharedctypes"~ (default), ~ctx~ also affects to shared memory management.                                                                                                                               |
...
because it requires interprocess communication.
...
**** Combination Effect of Context and Backend

| ~ctx~            | ~backend~      | internal data | synchronization objects | Comment         |
|------------------+----------------+---------------+-------------------------+-----------------|
| ~fork~ / ~spawn~ | ~sharedctypes~ | shared memory | OS semaphore            | Default         |
| ~fork~ / ~spawn~ | ~SharedMemory~ | shared memory | OS semaphore            | Not recommended |
| ~SyncManager~    | ~shreadctypes~ | proxy based   | proxy based             | Not recommended |
| ~SyncManager~    | ~ShreadMemory~ | shared memory | proxy based             | For [[https://ray.io/][Ray]] etc.    |
...
we use [[https://sphinx-automodapi.readthedocs.io/en/latest/][sphinx-automodapi]] to build the module structure. Unfortunately,
there is an unfixed bug [[https://gitlab.com/ymd_h/cpprb/-/issues/118][#118]], all methods are described twice (Why?).
...
[[https://ray.io/][Ray]] is a OSS framework which allows users to build distributed
application easily.

Ray can utilize multiple machines, so that Ray architecture doesn't
use shared memory except for immutable ~numpy.ndarray~. However, if
you use only a single machine, you might want to use shared memory for
replay buffer to avoid expensive inter process data sharing.

With cpprb 10.6+ and Python 3.8+, you can use ~MPReplayBuffer~ and
~MPPrioritizedReplayBuffer~ with Ray.

A key trick is to set ~authkey~ inside Ray process, which allows Ray
workers to communicate with ~SyncManager~ process.
...
The authors devide segment tree lock into global lock and last level
...
| Priority Update                               | Write the last level and update intermediate levels with lock.        | Write the last level parallelly. Update intermidiate levels with lock just before ~sample~ to avoid inconsistency.                                                                                                                                                                                            |
