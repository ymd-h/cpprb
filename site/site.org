...
| =eps=        | =float= | Small positive constant to avoid 0 priority            | =1e-6=  |


After construction, these classes can be used as functor. You can call
with =priorities= keyword and any other environment values.
...
The following psuedo code shows usage.
...
implementation don't have it. If you don't add small positive
...
caluculation, we implement LaBER separetely from replay buffer.

Then =LaBERmean= etc. become simple classes, so that these are ordinal
Python classes insted of Cython =cdef= classes.
...
upload Windows binary to PyPI. When we fix the macOS build problem, we
will upload the macOS binary, too.
