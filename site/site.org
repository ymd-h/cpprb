...
ratio of gradient update to environment transition, is a good factor
to adjust the oldest age of transition stored in the Replay Buffer.
...
requires increase of replay ratio. Performance also improves when
reducing the oldest age of transition while fixing replay buffer
capacity, which also requires increase of replay ratio.

If the capacity is increased without changing replay ratio, the
results vary. There are competitive 2 effects, improvement by
increasing the capacity and deterioration by the older age of
transition.


\(n\)-step configuration enlarges the amount of improvement with
larger replay buffer capacity.
