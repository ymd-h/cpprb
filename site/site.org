...
\[ \mathcal{L}^{\tau}_{\text{PER}}(\delta(i)) = \frac{\eta N}{\tau + \alpha - \alpha\beta}|\delta(i)|^{\tau + \alpha - \alpha\beta},~\eta = \frac{\min _j \mid \delta(i) \mid^{\alpha\beta}}{\sum _j |\delta(i)|^{\alpha}} \]
...
\[ p(i) = \frac{\max(|\delta(i)|^{\alpha},1)}{\sum _j \max(\mid\delta(j)\mid^{\alpha},1)},~\mathcal{L}_{text{Huber}}(\delta(i)) = \begin{cases} 0.5\delta(i)^2 & \text{if} |\delta(i)| \leq 1 \cr |\delta(i)| & \text{otherwise} \end{cases} \]

MSE is used below 1 because of smooth gradient around 0. By using
\(\max(|\delta(i)|^{\alpha},1)\), transitions around 0 TD errors
