...
These memory compression relies on the internal memory alignment, so
...
of ~str~), the "next value" of specified values are also set in the
...
wrap arounded if necessary), then are checked whether they are on the
