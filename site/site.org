...
prioritization (\(p_i\)). There are 2 variants of PER, proportional-base
\(p_i=\mid\delta_i\mid+\epsilon\) and rank-base
\(p_i=\frac{1}{\text{rank}(i)}\), where \(\epsilon\) is a small positive
value to avoid zero prioritization and \(\text{rank}(i)\) is rank of
transition \(i\) when transitions are sorted according to
