...
**** Backend Detail
~SharedMemory~ was introduced at Python 3.8. The backend can be
...
Python program correctly. This is often the case inside docker
container. You can increase the size by
...
The advantage of ~sharedctypes~ is fallback mechanism which can create
shared memory even when ~/dev/shm~ directory doesn't have enough
space.
...
We also have to select ~SharedMemory~ backend and ~SyncManager~ context.
...
Finnaly, (pseudo) example is as follows;
