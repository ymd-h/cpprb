...
** Overview
Model-augumented Prioritized Experience Replay (MaPER), which was
...
Network (MaCN), predict not only Q-value but also reward and next
...
$\sigma_i = \xi_1 |\delta Q_{\theta}|_{MSE} + \xi_2 |\delta R_{\theta}|_{MSE} + \xi_3 |\delta S_{\theta}|_{MSE}$.
