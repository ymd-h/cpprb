...
use shared memory except for immutable ~numpy.ndarray~.

However, if you use only a single machine, you might want to use
shared memory for replay buffer to avoid expensive inter process data
sharing.
...
#+INCLUDE: "../example/multiprocessing-ray.py"
