...
The concept is similar to V-trace and Retrace, however, LFIW doesn't
require episodic treatment and estimates probability ratio
\(d^{\pi}/d^{\mathcal{D}}\) by using extra network \(w_{\psi}(s, a)\).

The estimation is based on the following Lemma;

\[ D(P\|Q) \geq \mathbb{E}_P[f^{\prime}(w(x))] - \mathbb{E}_Q[f^{\ast}(f^{\prime}(w(x)))] \]

The equality is achieved when \( w = dP/dQ \).
