...
** TODO Multiprocess Learning (Ape-X)
:PROPERTIES:
...
(single learner with multiple explorers) efficiently.
...
its critical section is only index fetching and increment. (We will
describe segment tree for PER later.) This locking reduction allows
multiple explorers to add transitions parallelly.
