...
\[ p(i) = \frac{\max(|\delta(i)|^{\alpha},1)}{\sum _j \max(|\delta(j)|^{\alpha},1)},~\mathcal{L}_{text{Huber}}(\delta(i)) = \begin{cases} 0.5\delta(i)^2 & \text{if} |\delta(i)| \leq 1 \cr |\delta(i)| & \text{otherwise} \end{cases} \]
