...
\(\hat{p}_i \prompt \| \partial L(q_i, y_i) / \partial q_i \| \).
...
problems at PER is that the priorities are always outdated. Hoever,
re-computing priorities of all transitions in the buffer at every time
step is too expensive.

LaBER first samples \(m\)-times larger batch from the buffer
uniformly, then compute surrogate priorities for them, and samples
