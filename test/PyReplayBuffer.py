...
class TestMultiProcessPrioritizedReplayBuffer(TestPrioritizedReplayBuffer):
    ...
    @classmethod
    def setUpClass(cls):
        cls.rb = ThreadSafePrioritizedReplayBuffer(cls.buffer_size,
                                                   cls.obs_dim,
                                                   cls.act_dim,
                                                   alpha = cls.alpha)
        ...
        def f(end):
            ...
            next_obs = np.ones(shape=(cls.obs_dim))
            for i in range(0,end):
                cls.rb.add(obs,act,1.0,next_obs, 0,0.5)
            cls.rb.clear()
        ...
        def g(end):
            ...
            ps = np.ones((cls.add_dim)) * 5.0
            for i in range(0,end,cls.add_dim):
                cls.rb.add(obs,act,rew,next_obs,done,ps)
            cls.rb.clear()
