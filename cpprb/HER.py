...
class HindsightReplayBuffer:
    ...
    def on_episode_end(self, goal):
        ...
        rew = self.reward_func(trajectory[self.next_state], trajectory[self.action], goal)
        ...
        if self.strategy == "future":
            ...
            for i in range(episode_len):
                idx[:, i] = self.rng.integers(low=i, high=episode_len, size=self.additional_goals)
            ...
            idx = self.rng.integers(low=0, high=episode_len, size=(self.additional_goals, episode_len))
            for _i in idx:
                goal = self.goal_func(trajectory[self.next_state][_i])
                rew = self.reward_func(trajectory[self.next_state], trajectory[self.action], goal)
                self.rb.add(**trajectory, rew=rew, goal=goal)
        elif self.strategy == "final":
            goal = self.goal_func(np.broadcast_to(trajectory[self.next_state][-1], trajectory[self.next_state].shape))
            rew = self.reward_func(trajectory[self.next_state], trajectory[self.action], goal)
            self.rb.add(**trajectory, rew=rew, goal=goal)
        else:  # random
            # Note 1:
            ...
            #   strategy comparison.
            idx = self.rng.integers(low=0, high=self.rb.get_stored_size(), size=self.additional_goals * episode_len)
            goal = self.goal_func(self.rb._encode_sample(idx)[self.next_state])  # noqa: SLF001
            goal = goal.reshape((self.additional_goals, episode_len, *(goal.shape[1:])))
            for g in goal:
                rew = self.reward_func(trajectory[self.next_state], trajectory[self.action], g)
