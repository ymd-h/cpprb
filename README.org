...
is convenient when we move batch transitions from local buffers to a
global buffer. Moreover this is more efficient because of not only
removing slow Python for-loop but also suppressing unnecessary
